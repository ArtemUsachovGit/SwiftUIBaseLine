...
extension Publisher where Output == (data: Data, response: URLResponse) {
    func assumHTTP() -> AnyPublisher<(data: Data, response: HTTPURLResponse), HTTPError> {
        self.tryMap { (data: Data, response: URLResponse) -> (Data, HTTPURLResponse) in
...
extension Publisher where Output == (data: Data, response: HTTPURLResponse), Failure == HTTPError {
    func responseData() -> AnyPublisher<Data, HTTPError> {
        tryMap { (data: Data, response: HTTPURLResponse) -> Data in
            ...
            default:
                fatalError("Unhadled HTTP Response status code \(response.statusCode)")
            }
        }.mapError { error in
            return error as! HTTPError
        }.eraseToAnyPublisher()
